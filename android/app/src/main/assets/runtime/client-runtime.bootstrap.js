...
var layout=String(khRuntimeValues.khLayout||'');
...
storages.forEach(function(storage){setValue(storage,kh.paperTypeStorageKey,paper);setValue(storage,kh.layoutPresetStorageKey,layout);});
window.location.replace(redirect);
return;
}catch(e){console.error('kh bootstrap failed',e);}
}
kh.bootOnce().then(function(){return kh.refreshCurrentPage(false);}).catch(function(){return null;});window.addEventListener('DOMContentLoaded',function(){kh.schedulePageActionRefresh(false);},{once:true});window.addEventListener('pageshow',function(){kh.schedulePageActionRefresh(false);});window.addEventListener('hashchange',function(){window.dispatchEvent(new CustomEvent('kh:routeChanged',{detail:{type:'hashchange'}}));kh.schedulePageActionRefresh(false);});window.addEventListener('popstate',function(){window.dispatchEvent(new CustomEvent('kh:routeChanged',{detail:{type:'popstate'}}));kh.schedulePageActionRefresh(false);});
})();
