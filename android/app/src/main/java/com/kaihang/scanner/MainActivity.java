...
public class MainActivity extends BridgeActivity {
    ...
    private String buildClientRuntimeScript(String currentUrl) {
        ...
        script.append("kh.bootState=kh.bootState||'idle';kh.bootInstalled=!!kh.bootInstalled;kh.pageApplyState=kh.pageApplyState||'idle';kh.pageApplySignature=kh.pageApplySignature||'';kh.actionCatalogVersion=kh.actionCatalogVersion||0;kh.uiReadySignature=kh.uiReadySignature||'';");
        ...
        script.append("kh.resetUiReadyState=function(reason){kh.pushLog('重置页面就绪状态: '+String(reason||'ui reset'),'info');kh.uiReadySignature='';if(kh.pageApplyState==='ready'){kh.setPageApplyState('loading',reason||'ui reset');}};");
        ...
        script.append("kh.markUiReady=function(detail){var host=kh.findUiReadyHost();if(!host){kh.pushLog('页面就绪检查未命中: detail='+(detail||'')+', readyState='+document.readyState+', bodyChildren='+(document.body&&document.body.children?document.body.children.length:0),'warn');return false;}var hostDesc=kh.describeReadyHost(host);var selector=kh._lastUiReadySelector||'<unknown>';var signature=[window.location.pathname+window.location.search+window.location.hash,hostDesc,String(host.childElementCount||0),detail||'ready'].join('@@');if(signature===kh.uiReadySignature){kh.pushLog('页面就绪检查跳过重复: selector='+selector+', host='+hostDesc+', detail='+(detail||''),'plain');return true;}kh.uiReadySignature=signature;kh.pushLog('页面渲染就绪: selector='+selector+', host='+hostDesc+', detail='+(detail||''),'ok');kh.setPageApplyState('ready',detail||hostDesc);kh.schedulePageActionRefresh(false);return true;};");
        script.append("kh.installUiReadySignals=function(){if(kh._uiReadySignalsInstalled){kh.pushLog('页面就绪 observer 已安装，跳过重复安装','plain');return;}kh._uiReadySignalsInstalled=true;kh.pushLog('开始安装页面就绪 observer','info');var observer=null;var checkTimer=null;var scheduleCheck=function(detail){kh.pushLog('页面就绪检查排队: detail='+(detail||''),'plain');if(checkTimer)clearTimeout(checkTimer);checkTimer=setTimeout(function(){checkTimer=null;requestAnimationFrame(function(){requestAnimationFrame(function(){kh.markUiReady(detail||'mutation');});});},80);};var ensureObserver=function(){if(observer){kh.pushLog('页面就绪 observer 已存在','plain');return;}if(!window.MutationObserver){kh.pushLog('页面就绪 observer 不可用: MutationObserver missing','err');return;}if(!document.documentElement){kh.pushLog('页面就绪 observer 等待 documentElement','warn');return;}observer=new MutationObserver(function(mutations){for(var i=0;i<mutations.length;i++){var mutation=mutations[i];if(mutation.type==='childList'&&((mutation.addedNodes&&mutation.addedNodes.length)||(mutation.removedNodes&&mutation.removedNodes.length))){kh.pushLog('页面就绪 observer 命中 childList: added='+(mutation.addedNodes?mutation.addedNodes.length:0)+', removed='+(mutation.removedNodes?mutation.removedNodes.length:0),'plain');scheduleCheck('mutation childList');return;}}});observer.observe(document.documentElement,{childList:true,subtree:true,attributes:false});kh.pushLog('页面就绪 observer 已启动','info');};if(document.documentElement)ensureObserver();else window.addEventListener('DOMContentLoaded',ensureObserver,{once:true});if(document.readyState==='complete'||document.readyState==='interactive'){scheduleCheck('document '+document.readyState);}else{window.addEventListener('DOMContentLoaded',function(){scheduleCheck('DOMContentLoaded');},{once:true});}window.addEventListener('load',function(){scheduleCheck('window load');},{once:true});window.addEventListener('pageshow',function(){kh.resetUiReadyState('pageshow');scheduleCheck('pageshow');});window.addEventListener('kh:routeChanged',function(){kh.resetUiReadyState('route changed');scheduleCheck('route changed');});window.addEventListener('kh:storageChanged',function(evt){var key=String(evt&&evt.detail&&evt.detail.key||'');if(/NOCOBASE_(MAIN_)?(TOKEN|AUTH|ROLE)$/i.test(key)){kh.resetUiReadyState('auth changed');scheduleCheck('auth changed');}});};");
        ...
        script.append("kh.installActionObserver=function(){if(kh._actionObserverInstalled){kh.pushLog('页面动作 observer 已安装，跳过重复安装','plain');return;}kh._actionObserverInstalled=true;kh.pushLog('开始安装页面动作 observer','info');var observer=null;var refreshTimer=null;var scheduleRefresh=function(force){kh.pushLog('页面动作刷新排队: force='+(!!force),'plain');if(refreshTimer)clearTimeout(refreshTimer);refreshTimer=setTimeout(function(){refreshTimer=null;kh.schedulePageActionRefresh(!!force);},120);};var resubscribe=function(){if(observer){kh.pushLog('页面动作 observer 已存在','plain');return;}if(!window.MutationObserver||!document.documentElement){kh.pushLog('页面动作 observer 等待 DOM 环境','warn');return;}observer=new MutationObserver(function(mutations){var hasMeaningfulChange=false;for(var i=0;i<mutations.length;i++){var mutation=mutations[i];if(mutation.type==='childList'&&((mutation.addedNodes&&mutation.addedNodes.length)||(mutation.removedNodes&&mutation.removedNodes.length))){hasMeaningfulChange=true;kh.pushLog('页面动作 observer 命中 childList: added='+(mutation.addedNodes?mutation.addedNodes.length:0)+', removed='+(mutation.removedNodes?mutation.removedNodes.length:0),'plain');break;}if(mutation.type==='attributes'){hasMeaningfulChange=true;kh.pushLog('页面动作 observer 命中 attributes','plain');break;}}if(hasMeaningfulChange)scheduleRefresh(false);});observer.observe(document.documentElement,{childList:true,subtree:true,attributes:false});kh.pushLog('页面动作 observer 已启动','info');};if(document.documentElement)resubscribe();else window.addEventListener('DOMContentLoaded',resubscribe,{once:true});window.addEventListener('kh:routeChanged',function(){scheduleRefresh(false);});window.addEventListener('kh:storageChanged',function(evt){var key=String(evt&&evt.detail&&evt.detail.key||'');if(/NOCOBASE_(MAIN_)?(TOKEN|AUTH|ROLE)$/i.test(key)){scheduleRefresh(true);}});};");
