...
public class MainActivity extends BridgeActivity {
    ...
    private String buildClientRuntimeScript(String currentUrl) {
        ...
        script.append("kh.installUiReadySignals=function(){if(kh._uiReadySignalsInstalled)return;kh._uiReadySignalsInstalled=true;kh.pushLog('开始安装页面就绪 observer','info');var checkTimer=null;var scheduleCheck=function(detail){if(checkTimer)clearTimeout(checkTimer);checkTimer=setTimeout(function(){checkTimer=null;requestAnimationFrame(function(){requestAnimationFrame(function(){kh.markUiReady(detail||'mutation');});});},80);};var ensureObserver=function(){if(kh._uiReadyObserver||kh._uiReadyObserved)return;if(!window.MutationObserver||!document.documentElement)return;kh._uiReadyObserver=new MutationObserver(function(mutations){for(var i=0;i<mutations.length;i++){var mutation=mutations[i];if(mutation.type==='childList'&&((mutation.addedNodes&&mutation.addedNodes.length)||(mutation.removedNodes&&mutation.removedNodes.length))){scheduleCheck('mutation childList');return;}}});kh._uiReadyObserver.observe(document.documentElement,{childList:true,subtree:true,attributes:false});kh.pushLog('页面就绪 observer 已启动','info');scheduleCheck('observer started');};if(document.documentElement)ensureObserver();else window.addEventListener('DOMContentLoaded',ensureObserver,{once:true});if(document.readyState==='complete'||document.readyState==='interactive'){scheduleCheck('document '+document.readyState);}else{window.addEventListener('DOMContentLoaded',function(){scheduleCheck('DOMContentLoaded');},{once:true});}window.addEventListener('load',function(){scheduleCheck('window load');},{once:true});window.addEventListener('pageshow',function(){kh.resetUiReadyState('pageshow');ensureObserver();scheduleCheck('pageshow');});window.addEventListener('kh:routeChanged',function(){kh.resetUiReadyState('route changed');ensureObserver();scheduleCheck('route changed');});window.addEventListener('kh:storageChanged',function(evt){var key=String(evt&&evt.detail&&evt.detail.key||'');if(/NOCOBASE_(MAIN_)?(TOKEN|AUTH|ROLE)$/i.test(key)){kh.resetUiReadyState('auth changed');ensureObserver();scheduleCheck('auth changed');}});};");
