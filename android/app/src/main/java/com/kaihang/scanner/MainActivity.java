...
public class MainActivity extends BridgeActivity {
    ...
    private String buildClientRuntimeScript(String currentUrl) {
        ...
        script.append("kh.schedulePageActionRefresh=function(force){if(kh._pageActionRefreshScheduled)return;kh._pageActionRefreshScheduled=true;setTimeout(function(){kh._pageActionRefreshScheduled=false;kh.loadPageActions(!!force).catch(function(){return null;});},60);};");
        script.append("kh.patchHistory=function(){if(kh._historyPatched)return;kh._historyPatched=true;['pushState','replaceState'].forEach(function(name){var original=history[name];if(typeof original!=='function')return;history[name]=function(){var result=original.apply(this,arguments);kh.schedulePageActionRefresh(false);return result;};});};");
        ...
        script.append("kh.patchHistory();kh.attachButtonActions();kh.schedulePageActionRefresh(false);setTimeout(function(){kh.loadPageActions(false).catch(function(){return null;});},350);window.addEventListener('pageshow',function(){kh.schedulePageActionRefresh(false);});window.addEventListener('hashchange',function(){kh.schedulePageActionRefresh(false);});window.addEventListener('popstate',function(){kh.schedulePageActionRefresh(false);});");
        script.append("})();");
