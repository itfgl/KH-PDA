...
public class MainActivity extends BridgeActivity {
    ...
    private String buildClientRuntimeScript(String currentUrl) {
        ...
        script.append("kh.attachButtonActions=function(){if(window.__khButtonActionsBound)return;document.addEventListener('click',function(event){var actions=(window.__khPageActions&&window.__khPageActions.button)||[];if(actions.length){var rawTarget=event.target;var tag=rawTarget&&rawTarget.tagName?String(rawTarget.tagName).toLowerCase():'unknown';kh.pushLog('检测到点击事件: tag='+tag+', buttonActions='+actions.length,'plain');}for(var i=0;i<actions.length;i++){var action=actions[i];if(!action.triggerSelector){kh.pushLog('按钮动作缺少 triggerSelector: '+kh.describeAction(action),'warn');continue;}var target=event.target&&event.target.closest?event.target.closest(action.triggerSelector):null;if(!target){kh.pushLog('按钮动作未命中 selector: '+action.triggerSelector+'，'+(action.id||action.actionType),'plain');continue;}kh.pushLog('按钮动作命中 selector: '+action.triggerSelector+'，'+kh.describeAction(action),'ok');event.preventDefault();event.stopPropagation();kh.execAction(action,'');return;}},true);window.__khButtonActionsBound=true;};");
