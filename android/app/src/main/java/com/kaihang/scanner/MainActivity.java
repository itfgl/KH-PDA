...
public class MainActivity extends BridgeActivity {
    ...
    private void handleRuntimeInjection(WebView view, String targetUrl, InjectionTrigger trigger) {
        ...
        if (!shouldInjectForTrigger(trigger)) {
            appendNativeLog("跳过自动注入: mode=" + getCurrentInjectionMode() + ", trigger=" + trigger + ", url=" + url);
    ...
    private String buildClientRuntimeScript(String currentUrl) {
        ...
        script.append("kh.pushLog=function(msg,type){var text='['+new Date().toTimeString().slice(0,8)+'] '+String(msg||'');if(!kh.isFeatureEnabled('enableFloatingLogs',true))return text;try{var saved=JSON.parse((window.localStorage&&window.localStorage.getItem(kh.logStorageKey))||'[]');if(!Array.isArray(saved))saved=[];saved.push({text:text,type:type||'plain'});if(saved.length>200)saved=saved.slice(saved.length-200);window.localStorage&&window.localStorage.setItem(kh.logStorageKey,JSON.stringify(saved));window.__khLastLogSnapshot=saved;}catch(e){}kh.ensureFloatingLogger();kh.appendFloatingLog(text,type||'plain');return text;};");
