...
public class MainActivity extends BridgeActivity {
    ...
    private String buildClientRuntimeScript(String currentUrl) {
        ...
        script.append("kh.patchHistory();kh.patchStorage();kh.attachButtonActions();kh.installActionObserver();kh.schedulePageActionRefresh(false);window.addEventListener('DOMContentLoaded',function(){kh.schedulePageActionRefresh(false);},{once:true});window.addEventListener('pageshow',function(){kh.schedulePageActionRefresh(false);});window.addEventListener('hashchange',function(){window.dispatchEvent(new CustomEvent('kh:routeChanged',{detail:{type:'hashchange'}}));kh.schedulePageActionRefresh(false);});window.addEventListener('popstate',function(){window.dispatchEvent(new CustomEvent('kh:routeChanged',{detail:{type:'popstate'}}));kh.schedulePageActionRefresh(false);});");
        script.append("})();");
