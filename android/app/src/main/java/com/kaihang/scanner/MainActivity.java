...
public class MainActivity extends BridgeActivity {
    ...
    private String buildClientRuntimeScript(String currentUrl) {
        ...
        script.append("kh.resolveActionField=function(action,name,scanValue){var options=action&&action.options||{};var raw=action&&action.raw||{};var direct=options[name];if((direct===undefined||direct===null||String(direct).trim()==='')&&raw[name]!==undefined)direct=raw[name];if(direct!==undefined&&direct!==null&&String(direct).trim()!=='')return kh.applyTemplate(String(direct).trim(),scanValue);var selector=options[name+'_selector']||options[name+'Selector']||raw[name+'_selector']||raw[name+'Selector']||'';if(selector){var selected=kh.readSelector(selector);if(selected)return kh.applyTemplate(selected,scanValue);}if(name==='batch_no'&&scanValue)return String(scanValue).trim();if(name==='batch_no'&&action&&action.value)return kh.applyTemplate(action.value,scanValue);return '';};");
        ...
        script.append("kh.runPrintAction=function(action,scanValue){var plugin=kh.getPrintPlugin();if(!plugin)return Promise.reject(new Error('PrintPlugin unavailable'));var actionType=String(action.actionType||'').toLowerCase();if(actionType==='print_batch_label'||actionType==='pda_print'||actionType==='print_batch'){var payload={batchNo:kh.resolveActionField(action,'batch_no',scanValue),machineId:kh.resolveActionField(action,'machine_id',scanValue),productType:kh.resolveActionField(action,'product_type',scanValue),cavityNo:kh.resolveActionField(action,'cavity_no',scanValue),date:kh.resolveActionField(action,'date',scanValue),periodLabel:kh.resolveActionField(action,'period_label',scanValue),laneNo:kh.resolveActionField(action,'lane_no',scanValue)};if(!payload.batchNo)return Promise.reject(new Error('print_batch_label missing batchNo'));return kh.ensurePrintConnected(plugin).then(function(){return Promise.resolve(plugin.prepareToPrintLabel?plugin.prepareToPrintLabel():null).catch(function(){return null;});}).then(function(){var waitDone=kh.waitPrintStatus(plugin,'PRINT_OK',15000);return Promise.resolve(plugin.printBatchLabel(payload)).then(function(){return waitDone;});});}if(actionType==='print_machine_label'||actionType==='print_machine_qr'){var machinePayload={machineId:kh.resolveActionField(action,'machine_id',scanValue),productType:kh.resolveActionField(action,'product_type',scanValue),date:kh.resolveActionField(action,'date',scanValue)};if(!machinePayload.machineId)return Promise.reject(new Error('print_machine_label missing machineId'));return kh.ensurePrintConnected(plugin).then(function(){return Promise.resolve(plugin.printMachineQR(machinePayload));});}return Promise.reject(new Error('unsupported print action: '+actionType));};");
        script.append("kh.execAction=function(action,scanValue){if(!action||!action.enabled)return false;var runner=function(){if(action.actionType==='fill_input'||action.actionType==='fill'||action.actionType==='scan_fill'||action.actionType==='input'){return kh.injectValue(action.targetSelector,scanValue||action.value||'',!!action.autoPressEnter);}if(action.actionType==='click'||action.actionType==='tap'){return kh.clickSelector(action.targetSelector);}if(action.actionType==='print_batch_label'||action.actionType==='pda_print'||action.actionType==='print_batch'||action.actionType==='print_machine_label'||action.actionType==='print_machine_qr'){kh.runPrintAction(action,scanValue||'').then(function(){console.info('kh print action ok',action.id||action.actionType);}).catch(function(err){console.warn('kh print action failed',err);window.__khLastActionError=String(err&&err.message||err||'print failed');});return true;}if(action.actionType==='noop'||action.actionType==='none'){return true;}return false;};if(action.delayMs>0){setTimeout(runner,action.delayMs);return true;}return runner();};");
