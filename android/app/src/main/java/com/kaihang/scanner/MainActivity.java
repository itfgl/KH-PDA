...
public class MainActivity extends BridgeActivity {
    ...
    private String buildClientRuntimeScript(String currentUrl) {
        ...
        script.append("kh.isInternalHttpUrl=function(url){if(!url)return false;try{var next=new URL(url,window.location.href);if(!/^https?:$/i.test(next.protocol))return false;return next.origin===window.location.origin;}catch(e){return false;}};");
        script.append("kh.navigateInApp=function(url){if(!url)return false;try{var next=new URL(url,window.location.href).toString();kh.pushLog('拦截站内跳转: '+next,'info');window.location.assign(next);return true;}catch(e){kh.pushLog('站内跳转失败: '+String(e&&e.message||e||'unknown'),'err');return false;}};");
        script.append("kh.patchWindowOpen=function(){if(window.__khWindowOpenPatched)return;var original=window.open;window.open=function(url,target,features){if(kh.isInternalHttpUrl(url)){kh.navigateInApp(url);return window;}kh.pushLog('外部打开请求: '+String(url||''),'warn');if(typeof original==='function'){return original.call(window,url,target,features);}return null;};window.__khWindowOpenPatched=true;};");
        script.append("kh.patchAnchorNavigation=function(){if(window.__khAnchorPatched)return;document.addEventListener('click',function(event){var anchor=event.target&&event.target.closest?event.target.closest('a[href]'):null;if(!anchor)return;var href=anchor.getAttribute('href')||'';var target=(anchor.getAttribute('target')||'').toLowerCase();var rel=(anchor.getAttribute('rel')||'').toLowerCase();var wantsExternal=target==='_blank'||rel.indexOf('external')>=0||anchor.hasAttribute('download');if((wantsExternal||kh.isInternalHttpUrl(href))&&kh.isInternalHttpUrl(href)){event.preventDefault();event.stopPropagation();kh.navigateInApp(href);}},true);window.__khAnchorPatched=true;};");
