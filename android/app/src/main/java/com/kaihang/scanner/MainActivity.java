...
public class MainActivity extends BridgeActivity {
    ...
    private String buildClientRuntimeScript(String currentUrl) {
        ...
        script.append("kh.applyClientConfig=function(config){config=config||{};var serverBase=kh.normalizeBaseUrl(config.serverBase,kh.getCurrentHttpOrigin()||kh.defaultServerBase);var updateBase=kh.normalizeBaseUrl(config.updateBase,serverBase);var paperType=String(config.paperType||'thermal').trim().toLowerCase()==='black_mark'?'black_mark':'thermal';var layoutPreset=String(config.layoutPreset||'standard').trim().toLowerCase();if(['compact','large'].indexOf(layoutPreset)<0)layoutPreset='standard';kh.clientConfig={serverBase:serverBase,updateBase:updateBase,paperType:paperType,layoutPreset:layoutPreset};return kh.clientConfig;};");
        ...
        script.append("kh.readClientConfig=function(){var plugin=kh.getClientConfigPlugin();if(!plugin||!plugin.getConfig)return Promise.resolve(kh.clientConfig);return Promise.resolve(plugin.getConfig()).then(function(config){return kh.applyClientConfig(config);}).catch(function(){return kh.clientConfig;});};");
        ...
        script.append("kh.ensureSettingsPanel=function(){if(document.getElementById('kh-settings-fab'))return;var mount=function(){if(document.getElementById('kh-settings-fab')||!document.body)return;if(!document.getElementById('kh-settings-style')&&document.head){var style=document.createElement('style');style.id='kh-settings-style';style.textContent='.kh-settings-fab{position:fixed;left:18px;bottom:22px;z-index:2147483000;width:54px;height:54px;border:none;border-radius:999px;background:#111827;color:#fff;font-size:14px;font-weight:700;box-shadow:0 8px 24px rgba(0,0,0,.24)}.kh-settings-overlay{position:fixed;inset:0;z-index:2147483002;background:rgba(15,23,42,.42);display:none;align-items:center;justify-content:center;padding:18px}.kh-settings-panel{width:min(100%,380px);background:#fff;border-radius:18px;padding:16px;box-shadow:0 16px 40px rgba(15,23,42,.24)}.kh-settings-title{font-size:13px;font-weight:800;color:#667085;letter-spacing:.08em;text-transform:uppercase;margin-bottom:12px}.kh-settings-field{margin-bottom:12px}.kh-settings-field label{display:block;font-size:12px;font-weight:600;color:#667085;margin-bottom:6px}.kh-settings-field input,.kh-settings-field select{width:100%;padding:12px 13px;border:1px solid #d0d5dd;border-radius:12px;background:#fff;font-size:15px;outline:none}.kh-settings-field input:focus,.kh-settings-field select:focus{border-color:#1570ef;box-shadow:0 0 0 3px rgba(21,112,239,.12)}.kh-settings-note{font-size:12px;line-height:1.6;color:#667085;margin:8px 0 12px}.kh-settings-status{display:none;border-radius:12px;padding:10px 12px;font-size:13px;line-height:1.5;background:#f2f4f7;color:#344054;margin-bottom:10px}.kh-settings-status.ok{display:block;background:#dcfae6;color:#166534}.kh-settings-status.err{display:block;background:#fee4e2;color:#b42318}.kh-settings-status.info{display:block;background:#dbeafe;color:#1d4ed8}.kh-settings-row{display:flex;gap:8px}.kh-settings-btn{flex:1;border:none;border-radius:12px;padding:12px 13px;font-size:14px;font-weight:700;cursor:pointer}.kh-settings-btn.primary{background:#1570ef;color:#fff}.kh-settings-btn.secondary{background:#eaecf0;color:#101828}';document.head.appendChild(style);}var fab=document.createElement('button');fab.id='kh-settings-fab';fab.type='button';fab.className='kh-settings-fab';fab.textContent='设置';var overlay=document.createElement('div');overlay.id='kh-settings-overlay';overlay.className='kh-settings-overlay';overlay.innerHTML='<div class=\"kh-settings-panel\"><div class=\"kh-settings-title\">客户端设置</div><div id=\"kh-settings-status\" class=\"kh-settings-status\"></div><div class=\"kh-settings-field\"><label>服务地址</label><input id=\"kh-settings-server\" type=\"url\" inputmode=\"url\" autocomplete=\"url\" placeholder=\"http://127.0.0.1:13000\"></div><div class=\"kh-settings-field\"><label>更新地址</label><input id=\"kh-settings-update\" type=\"url\" inputmode=\"url\" autocomplete=\"url\" placeholder=\"http://127.0.0.1:13000\"></div><div class=\"kh-settings-field\"><label>纸张类型</label><select id=\"kh-settings-paper\"><option value=\"thermal\">普通热敏纸</option><option value=\"black_mark\">黑标标签纸</option></select></div><div class=\"kh-settings-field\"><label>排版预设</label><select id=\"kh-settings-layout\"><option value=\"standard\">标准排版</option><option value=\"compact\">紧凑排版</option><option value=\"large\">大字排版</option></select></div><div class=\"kh-settings-note\">配置会写入 Android 本地并重启应用，服务地址与更新地址都会按新配置生效。</div><div class=\"kh-settings-row\"><button type=\"button\" class=\"kh-settings-btn primary\" id=\"kh-settings-save\">保存并重启</button><button type=\"button\" class=\"kh-settings-btn secondary\" id=\"kh-settings-close\">关闭</button></div></div>';document.body.appendChild(fab);document.body.appendChild(overlay);kh._settingsOverlay=overlay;kh._settingsStatus=overlay.querySelector('#kh-settings-status');var fill=function(){overlay.querySelector('#kh-settings-server').value=kh.getServerBase();overlay.querySelector('#kh-settings-update').value=kh.getUpdateBase();overlay.querySelector('#kh-settings-paper').value=kh.getPrintPaperType();overlay.querySelector('#kh-settings-layout').value=kh.getPrintLayoutPreset();kh.setSettingsStatus('', '');};kh.setSettingsStatus=function(message,type){if(!kh._settingsStatus)return;if(!message){kh._settingsStatus.style.display='none';kh._settingsStatus.textContent='';kh._settingsStatus.className='kh-settings-status';return;}kh._settingsStatus.style.display='block';kh._settingsStatus.textContent=message;kh._settingsStatus.className='kh-settings-status '+(type||'info');};kh.openSettingsPanel=function(){kh.readClientConfig().then(function(){fill();overlay.style.display='flex';});};kh.closeSettingsPanel=function(){overlay.style.display='none';};fab.addEventListener('click',function(){kh.openSettingsPanel();});overlay.addEventListener('click',function(evt){if(evt.target===overlay)kh.closeSettingsPanel();});overlay.querySelector('#kh-settings-close').addEventListener('click',function(){kh.closeSettingsPanel();});overlay.querySelector('#kh-settings-save').addEventListener('click',function(){var serverBase=kh.normalizeBaseUrl(overlay.querySelector('#kh-settings-server').value,kh.defaultServerBase);var updateBase=kh.normalizeBaseUrl(overlay.querySelector('#kh-settings-update').value,serverBase);var paperType=overlay.querySelector('#kh-settings-paper').value==='black_mark'?'black_mark':'thermal';var layoutPreset=overlay.querySelector('#kh-settings-layout').value;if(!serverBase){kh.setSettingsStatus('请输入服务地址','err');return;}if(!updateBase){kh.setSettingsStatus('请输入更新地址','err');return;}var settings={serverBase:serverBase,updateBase:updateBase,paperType:paperType,layoutPreset:layoutPreset};var plugin=kh.getClientConfigPlugin();kh.applyClientConfig(settings);if(!plugin||!plugin.saveConfig){kh.setSettingsStatus('当前环境缺少原生配置插件，改为直接跳转','info');setTimeout(function(){window.location.assign(serverBase);},300);return;}Promise.resolve(plugin.saveConfig(settings)).then(function(saved){kh.applyClientConfig(saved||settings);kh.setSettingsStatus('配置已保存，应用即将重启…','info');setTimeout(function(){if(plugin.restartApp){Promise.resolve(plugin.restartApp()).catch(function(){window.location.assign(kh.getServerBase());});}else{window.location.assign(kh.getServerBase());}},350);}).catch(function(err){var message=String(err&&err.message||err||'unknown');kh.setSettingsStatus('保存失败: '+message,'err');});});};if(document.body)mount();else window.addEventListener('DOMContentLoaded',mount,{once:true});};");
        ...
        script.append("kh.isHttpUrl=function(url){if(!url)return false;try{var next=new URL(url,window.location.href);return /^https?:$/i.test(next.protocol);}catch(e){return false;}};");
        script.append("kh.navigateInApp=function(url){if(!url)return false;try{var next=new URL(url,window.location.href).toString();kh.pushLog('拦截应用内跳转: '+next,'info');window.location.assign(next);return true;}catch(e){kh.pushLog('应用内跳转失败: '+String(e&&e.message||e||'unknown'),'err');return false;}};");
        script.append("kh.patchWindowOpen=function(){if(window.__khWindowOpenPatched)return;var original=window.open;window.open=function(url,target,features){var normalizedTarget=String(target||'').toLowerCase();if(kh.isHttpUrl(url)&&normalizedTarget!=='_system'){kh.navigateInApp(url);return window;}kh.pushLog('外部打开请求: '+String(url||''),'warn');if(typeof original==='function'){return original.call(window,url,target,features);}return null;};window.__khWindowOpenPatched=true;};");
        script.append("kh.patchAnchorNavigation=function(){if(window.__khAnchorPatched)return;document.addEventListener('click',function(event){var anchor=event.target&&event.target.closest?event.target.closest('a[href]'):null;if(!anchor)return;var href=anchor.getAttribute('href')||'';if(!kh.isHttpUrl(href))return;var rel=(anchor.getAttribute('rel')||'').toLowerCase();var forceExternal=rel.indexOf('external')>=0||anchor.hasAttribute('download')||anchor.getAttribute('data-kh-external')==='true';if(forceExternal)return;event.preventDefault();event.stopPropagation();kh.navigateInApp(href);},true);window.__khAnchorPatched=true;};");
        script.append("kh.patchWindowOpen();kh.patchAnchorNavigation();");
