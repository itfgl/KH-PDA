...
public class MainActivity extends BridgeActivity {
    ...
    private String buildClientRuntimeScript(String currentUrl) {
        ...
        script.append("kh.ensureUpdateButton();");
        ...
        script.append("kh.patchWindowOpen();kh.ensureDeviceClient();kh.pushLog('网页日志桥已启动 version='+String(window.APP_VERSION_NAME||'')+' ('+String(window.APP_VERSION_CODE||'')+'), buildTime='+String(window.BUILD_TIME||''),'info');kh.ensureScanBridge().catch(function(){return null;});kh.pushLog('页面注入完成: '+window.location.href,'plain');");
        ...
        script.append("kh.loadPageActions=function(){var storages=[window.localStorage,window.sessionStorage].filter(Boolean);var getStored=function(key){for(var i=0;i<storages.length;i++){var value=storages[i].getItem(key);if(value)return value;}return '';};var role=getStored('NOCOBASE_ROLE')||getStored('NOCOBASE_MAIN_ROLE')||'';if(kh._pageActionsLoading)return kh._pageActionsLoading;var token=getStored('NOCOBASE_TOKEN')||getStored('NOCOBASE_MAIN_TOKEN');var auth=getStored('NOCOBASE_AUTH')||getStored('NOCOBASE_MAIN_AUTH')||'basic';if(!token||!window.fetch){window.__khPageActions={scan:[],button:[]};window.__khExecTriggeredActions=kh.execTriggeredActions;kh.attachButtonActions();var bridge0=kh.getNativeBridge();if(bridge0&&bridge0.setScanActionEnabled)bridge0.setScanActionEnabled(false);kh.pushLog('未检测到页面 token，跳过页面动作加载','warn');return Promise.resolve();}var requestUrl=new URL(kh.pageActionsApi,window.location.origin).toString();kh._pageActionsLoading=window.fetch(requestUrl,{headers:{'Authorization':'Bearer '+token,'X-Authenticator':auth,'X-Requested-With':'XMLHttpRequest'}}).then(function(res){if(!res.ok)throw new Error('page actions '+res.status);return res.json();}).then(function(payload){var data=(payload&&payload.data!==undefined)?payload.data:payload;var items=Array.isArray(data)?data:(Array.isArray(data&&data.items)?data.items:(Array.isArray(data&&data.rows)?data.rows:[]));var scan=[];var button=[];for(var i=0;i<items.length;i++){var action=kh.normalizeAction(items[i],i+1);if(!action)continue;if(!action.enabled)continue;if(!kh.roleMatch(action.roleName,role))continue;if(!kh.pageMatch(action.pagePath,window.location.href))continue;if(action.triggerType==='button')button.push(action);else if(action.triggerType==='scan')scan.push(action);}scan.sort(function(a,b){return (a.sortOrder||0)-(b.sortOrder||0);});button.sort(function(a,b){return (a.sortOrder||0)-(b.sortOrder||0);});window.__khPageActions={scan:scan,button:button};window.__khExecTriggeredActions=kh.execTriggeredActions;kh.attachButtonActions();var bridge=kh.getNativeBridge();if(bridge&&bridge.setScanActionEnabled)bridge.setScanActionEnabled(scan.length>0);var signature=kh.getActionSignature(scan,button,role);var urlKey=window.location.pathname+window.location.search+window.location.hash;if(signature!==kh._lastPageActionSignature||urlKey!==kh._lastPageActionUrlKey){kh._lastPageActionSignature=signature;kh._lastPageActionUrlKey=urlKey;kh.pushLog('页面动作已加载: scan='+scan.length+', button='+button.length+', role='+(role||'<empty>'),'info');scan.forEach(function(action,index){kh.pushLog('已注册扫码动作['+(index+1)+'/'+scan.length+']: '+kh.describeAction(action),'info');});button.forEach(function(action,index){kh.pushLog('已注册按钮动作['+(index+1)+'/'+button.length+']: '+kh.describeAction(action),'info');});}return true;}).catch(function(err){kh.pushLog('页面动作加载失败: '+String(err&&err.message||err||'unknown'),'err');window.__khPageActions={scan:[],button:[]};window.__khExecTriggeredActions=kh.execTriggeredActions;kh.attachButtonActions();var bridge1=kh.getNativeBridge();if(bridge1&&bridge1.setScanActionEnabled)bridge1.setScanActionEnabled(false);return false;}).finally(function(){kh._pageActionsLoading=null;});return kh._pageActionsLoading;};");
        script.append("kh.schedulePageActionRefresh=function(){if(kh._pageActionRefreshScheduled)return;kh._pageActionRefreshScheduled=true;setTimeout(function(){kh._pageActionRefreshScheduled=false;kh.loadPageActions().catch(function(){return null;});},180);};");
        ...
        script.append("Promise.resolve().then(function(){return kh.loadPageActions();});setTimeout(function(){kh.loadPageActions().catch(function(){return null;});},600);setTimeout(function(){kh.loadPageActions().catch(function(){return null;});},1800);window.addEventListener('pageshow',function(){kh.schedulePageActionRefresh();});window.addEventListener('hashchange',function(){kh.schedulePageActionRefresh();});window.addEventListener('popstate',function(){kh.schedulePageActionRefresh();});");
        script.append("})();");
